...
- 长时任务权限：ohos.permission.KEEP_BACKGROUND_RUNNING
...
2. 引入朗读组件句柄。
   ...
   ```
   import { BarAlignment, DisplayTab, ReaderContent, ReaderParams, ResetParamType, TextReaderController, TextRead, IconParams } from 'module_text_reader';
   ```

3. 调用组件，详细组件调用参见[API参考](#API参考)。

4. （可选）如需要设置后台运行功能，请配置长时任务权限，在项目的entry模块下src/main/module.json5中添加配置。
   ```
   "abilities": [
      {
        // ...
