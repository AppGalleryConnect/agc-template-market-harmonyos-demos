...
| wxLogin     | (loginParams: [LoginParams](#LoginParams对象说明)) => Promise<void> | 否    | 微信登录成功事件  |
...
| anonymousPhone    | string                 | 否    | 匿名手机号                   |
| authorizationCode | string                 | 否    | 账号一键登录authorizationCode |
| unionID           | string                 | 否    | 联合ID                    |
| wxResp            | wxopensdk.SendAuthResp | 否    | 授权登录回调结果                |

### LoginSheetUtils

半模态登录。

#### static open(uiContext: UIContext, params: [LoginInfoOptions](#LoginInfoOptions对象说明)): void
...
obtainVerifyCode: (phone: string) => Promise<void>
...
struct AggregatedLoginSample1 {
  ...
  wxLogin = async (loginParams: LoginParams): Promise<void> => {
    ...
    Navigation(this.navPathStack) {
      Column() {
        ...
        AggregatedLogin({
          ...
          icon: $r('app.media.startIcon'),  // todo 需要替换为开发者所需的图像资源文件。
          themeColor: '#0A59F7',
          isSheet: false,
          autoSheet: false,
          isPersistence: false,
...
struct AggregatedLoginSample2 {
   ...
   build() {
      Column() {
         ...
         Button('点击登录')
            .onClick(() => {
               LoginSheetUtils.open(this.getUIContext(), this.params)
            })
