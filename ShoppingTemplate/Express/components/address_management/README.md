...
<div style='overflow-x:auto'>
  <table style='min-width:800px'>
    ...
    <tr>
      ...
      <td valign='top'><img src='screenshots/address_list_phone.png' style='max-height:350px;object-fit:contain'></td>
      <td valign='top'><img src='screenshots/address_list_foldable.png' style='max-height:350px;object-fit:contain'></td>
      <td valign='top'><img src='screenshots/address_list_tablet.png' style='max-height:350px;object-fit:contain'></td>
    </tr>
    <tr>
      <th scope='row'>地址编辑</th>
      <td valign='top'><img src='screenshots/address_edit_phone.png' style='max-height:350px;object-fit:contain'></td>
      <td valign='top'><img src='screenshots/address_edit_foldable.png' style='max-height:350px;object-fit:contain'></td>
      <td valign='top'><img src='screenshots/address_edit_tablet.png' style='max-height:350px;object-fit:contain'></td>
    </tr>
  </table>
</div>
...
5. 引入组件。
    ...
    ```typescript
    import { AddressController, AddressDTO, goToAddressListPage } from 'address_management';
    ```
...
| 字段名            | 类型                                     | 是否必填 | 说明                                   |
| ----------------- | ---------------------------------------- | -------- | -------------------------------------- |
| controller        | AddressController                        | 是       | 地址管理控制器，用于管理地址的增删改查 |
...
| 名称            | 类型                                                         | 是否必填 | 说明                 |
| --------------- | ------------------------------------------------------------ | -------- | -------------------- |
| addressList     | AddressVM[]                                                  | 否       | 当前地址列表数据。   |
| onLoadAddress   | () => Promise<AddressVM[]>                                   | 否       | 加载地址列表数据回调 |
| onAddAddress    | (item: AddressDTO) => Promise<boolean>                       | 否       | 添加地址回调         |
| onDeleteAddress | (addressId: number) => Promise<boolean>                      | 否       | 删除地址回调         |
| onModifyAddress | (address: AddressDTO, isSetDefault?: boolean) => Promise<boolean> | 否       | 修改地址回调         |
...
struct Index {
  ...
  aboutToAppear(): void {
    // 注入查询逻辑
    this.controller.onLoadAddress = async (): Promise<AddressVM[]> => {
      // todo 请求查询地址列表接口
      return this.addressListMock.map((item) => new AddressVM(item))
    };

    // 注入新增地址逻辑
    this.controller.onAddAddress = async (address: AddressDTO): Promise<boolean> => {
      // todo 请求新增地址接口
      const time: number = new Date().getTime();
      address.createdAt = time;
      address.updatedAt = time;
      this.addressListMock.push(address);
      return true;
    };

    // 注入删除逻辑
    this.controller.onDeleteAddress = async (addressId: number): Promise<boolean> => {
      // todo 请求删除地址接口
      const idx = this.addressListMock.findIndex((item) => item.addressId === addressId)
      if (idx > -1) {
        this.addressListMock.splice(idx, 1);
      }
      return true;
    };
    ...
    this.controller.onModifyAddress = async (address: AddressDTO, isSetDefault: boolean = false) => {
      const time = Date.now();
      address.updatedAt = time;
      if (isSetDefault || address.isDefault) {
         this.addressListMock.forEach((item) => {
            item.isDefault = false;
         })
      }
      const idx = this.addressListMock.findIndex((item) => item.addressId === address.addressId)
      if (idx > -1) {
        this.addressListMock.splice(idx, 1, address);
      }
      return true;
    }
  }
