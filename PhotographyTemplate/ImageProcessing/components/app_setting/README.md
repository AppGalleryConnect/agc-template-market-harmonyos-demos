...
checkAppUpdate(): Promise<updateManager.CheckUpdateResult | ErrorMessage>

检查应用市场是否有新的版本。本地安装版本须低于应用市场在架版本才能检查到更新，且签名信息须保持一致。

**返回值说明：**

| 类型 | 说明 |
|:-----|:-----|
| updateManager.CheckUpdateResult | 检查更新结果 |
| ErrorMessage | 错误信息 |
...
showCommentDialog(config?: CommentDialogConfig): Promise<boolean>

显示评论弹窗，支持时间间隔控制。默认间隔3天才会重新弹窗，也可强制弹出。

**参数说明：**

| 参数名 | 类型 | 是否必填 | 说明 |
|:------|:-----|:-------|:-----|
| config | CommentDialogConfig | 否 | 评论弹窗配置 |
...
setNoNewVersionTips(tips: string): AppServiceUtils

设置暂无最新版本的提示词，需要在调用 checkAppUpdate 前设置，否则使用默认提示词"已是最新版本"。
...
| 类型 | 说明 |
|:-----|:-----|
| AppServiceUtils | 返回当前实例，支持链式调用 |

---

##### setUpdateFailedTips

setUpdateFailedTips(tips: string): AppServiceUtils

设置升级失败的提示词，需要在调用 checkAppUpdate 前设置，否则使用默认提示词"升级版本失败~"。
...
setCheckUpdateListener(callbacks: CheckUpdateCallback): AppServiceUtils
...
| 参数名 | 类型 | 是否必填 | 说明 |
|:------|:-----|:-------|:-----|
| callbacks | CheckUpdateCallback | 是 | 回调函数集合 |
...
setUpdateListener(callbacks: UpdateCallback): AppServiceUtils
...
| 参数名 | 类型 | 是否必填 | 说明 |
|:------|:-----|:-------|:-----|
| callbacks | UpdateCallback | 是 | 回调函数集合 |
...
setCommentListener(callbacks: CommentCallback): AppServiceUtils
...
| 参数名 | 类型 | 是否必填 | 说明 |
|:------|:-----|:-------|:-----|
| callbacks | CommentCallback | 是 | 回调函数集合 |
...
| 参数名 | 类型 | 是否必填 | 说明 |
|:------|:-----|:-------|:-----|
| intervalDays | number | 否 | 评论弹窗间隔天数，默认值3 |
| forceShow | boolean | 否 | 是否强制弹出（忽略时间间隔），默认值false |
...
| 参数名 | 类型 | 是否必填 | 说明 |
|:------|:-----|:-------|:-----|
| onSuccess | (checkResult: updateManager.CheckUpdateResult) => void | 否 | 检查更新成功回调 |
| onFailed | (error: ErrorMessage) => void | 否 | 检查更新失败回调 |
...
| 参数名 | 类型 | 是否必填 | 说明 |
|:------|:-----|:-------|:-----|
| onSuccess | (resultCode: updateManager.ShowUpdateResultCode) => void | 否 | 升级成功回调 |
| onFailed | (error: ErrorMessage) => void | 否 | 升级失败回调 |
...
| 参数名 | 类型 | 是否必填 | 说明 |
|:------|:-----|:-------|:-----|
| onSuccess | () => void | 否 | 评论弹窗成功回调 |
| onFailed | (error: ErrorMessage) => void | 否 | 评论弹窗失败回调 |
...
```typescript
import { AppServiceUtils,ErrorMessage } from 'app_setting';

// 基础使用 - 检查更新
const appServices = new AppServiceUtils(this.getUIContext());
 appServices.checkAppUpdate();
// 链式调用 - 自定义提示词
 appServices
  .setNoNewVersionTips('当前已是最新版本')
  .setUpdateFailedTips('更新失败，请稍后重试')
  .checkAppUpdate();

// 显示评论弹窗（默认3天间隔）
 appServices.showCommentDialog();

// 显示评论弹窗（自定义7天间隔）
 appServices.showCommentDialog({ intervalDays: 7 });

// 强制显示评论弹窗（忽略时间间隔）
 appServices.showCommentDialog({ forceShow: true });

// 设置监听回调
 appServices
  .setCheckUpdateListener({
    onSuccess: () => console.log('检查成功', result),
    onFailed: (error: ErrorMessage) => console.log('检查失败', error)
  })
  .setCommentListener({
    onSuccess: () => console.log('评论弹窗成功'),
    onFailed: (error: ErrorMessage) => console.log('评论弹窗失败', error)
  })
  .checkAppUpdate();
```
