...
本组件提供了音频音调调节功能，支持选择音频文件，通过滑块调节音频的音调（赫兹）。

<img src='screenshots/audio_tone.jpeg' style='max-height:350px;object-fit:contain'>
...
1. 安装组件
   ...
   b. 在项目根目录 build-profile.json5 添加 audio_worker、audio_common、audio_adjust_tone 模块。

    ```
    // 在项目根目录 build-profile.json5 填写 audio_adjust_tone、audio_worker、audio_common 路径。其中 XXX 为组件存放的目录名
    "modules": [
    {
      "name": "audio_common",
    ...
    {
      "name": "audio_adjust_tone",
      "srcPath": "./XXX/audio_adjust_tone"
    },
    {
      "name": "audio_worker",
   ...
   c. 在项目根目录 oh-package.json5 中添加依赖。
    ...
    {
      "dependencies": {
          "audio_adjust_tone": "file:./XXX/audio_adjust_tone",
...
**AudioAdjustTone(params: [AudioAdjustToneParams](#AudioAdjustToneParams)): void**

音频调节组件，提供音频音调调节、保存等功能。

### AudioAdjustToneParams

AudioAdjustTone 组件的参数结构体。

| 字段名            | 类型                          | 是否必填 | 默认值                     | 说明                            |
|----------------|-----------------------------|----|-------------------------|-------------------------------|
| musicTitle     | string                      | 是  | ''                      | 音频文件名称                        |
| sourcePath     | string                      | 是  | ''                      | 源音频文件路径                       |
...
| resultCallBack | (outPath: string) => void   | 否  | (outPath: string) => {} | 音频调节完成后的回调函数，返回输出文件路径         |
