...
void PcmBufferQueue::Enqueue(const std::shared_ptr<PcmData>& data)
{
    ...
    bool status = condFull_.wait_for(lock, 1s, [this]() {
        return queue_.size() < capacity_ || cancelEnqueueFlag_ == true;
    });
    if (!status || cancelEnqueueFlag_) {
        return;
    }
    queue_.push(data);
...
{
    std::unique_lock<std::mutex> lock(mutex_);
    bool status = condEmpty_.wait_for(lock, 1s, [this]() { return !queue_.empty() || cancelDequeueFlag_ == true; });
    if (!status) {
        return nullptr;
    }
